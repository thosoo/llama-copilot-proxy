...
async function run() {
  ...
  const pythonPath = '/home/thaison/llama-copilot-proxy/.venv/bin/python';
  const proxy = spawn(pythonPath, ['/home/thaison/llama-copilot-proxy/proxy_server.py'], {
    env: { ...process.env, LISTEN_PORT: String(PROXY_PORT), UPSTREAM: `http://127.0.0.1:${UPSTREAM_PORT}`, THINKING_MODE: 'show_reasoning', VERBOSE: 'false' },
  ...
  console.log('✅ Python proxy separator injection test passed');
}
